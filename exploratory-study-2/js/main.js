...
// array for fixed scroll positions
var scrollPositions = [];
...
// populate fixed scroll positions array
...
function findClosestScrollPosition(){
  for(var i = 1; i < scrollPositions.length; i++){
    if(scrollPositions[i] > (actualScrollTop + 10)){
...
function updateWindowPosition(inc, dec){
  ...
  if(inc){
    ...
    if(scrollPositionsIndex >= scrollPositions.length){
      scrollPositionsIndex = scrollPositionsIndex % scrollPositions.length;
    }
  }else if(dec){
    scrollPositionsIndex--;
  ...
  var percentage = scrollPositionsIndex / (scrollPositions.length - 1) * 100;
...
$("#stepper").on("change", function(e){
  ...
  scrollPositionsIndex = Math.floor(sliderValue / 100 * (scrollPositions.length - 1))
  updateWindowPosition();
})
...
function showStepper(){
  ...
  $("#slider-total").text(scrollPositionNames.length);
...
window.onscroll = function(e) {
  if((window.innerHeight + window.scrollY) >= $(document).height()){
    ...
    // 3rd value is when we agreed to consent form, see how long it took to read!
...
// begin with chapter 1, only for study
// filterChapter("chap1");

// show settings for others
toggleSettings();
