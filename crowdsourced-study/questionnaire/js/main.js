// if user has been rejected, do not let them see content!
if(!canUserAccessPage()){
  rejectUser();
}

// data for storing our timing experiment
var pageTimingData = [];


// whether or not to filter story to one of three chapters, or not
var chapterFilter = false;

// variable to track scroll record positions
var recordedPositionsIndex = -1;


var Dispatcher = _.clone(Backbone.Events);

// prep dialog window for showing the resolution checker modal
var dialog = document.querySelector('#resolution-checker');
if(!dialog.showModal){
		dialogPolyfill.registerDialog(dialog);
  }

function performResolutionCheck(){
  // perform resolution check, only show story if right resolution
  if(isWindowBigEnough() && !isWindowTooBig()){
    try{
      dialog.close();
    }catch(error){}
    if(!stepper){
    ...
    recordTime("stop-checking-resolution");
  }else{
    recordTime("checking-resolution");
    try{
      dialog.showModal();
    }catch(error){}
    var resolutionChecker = $("iframe.resolution-checker").contents();
		resolutionChecker.contents().find(".browserHeight").text(window.innerHeight);
    resolutionChecker.contents().find(".browserWidth").text(window.innerWidth);
...
$window.on("resize", _.debounce(function() {
  ...
  // performResolutionCheck();

  if(!staticCondition){
    Dispatcher.trigger("resize");
    // bit of a hack to make sure stepper stays in correct place due to scrolling changes
    if(stepper){
      hideStepper();
      showStepper();
    }
  }else if(staticCondition){
    placeStaticImages();
  }
}, 700));

var R2D3Views = [];
// adjust fps to number of frames for an animation
var fps = 90;
var scrollTop = 0, actualScrollTop = 0;

// polled every so often, on scroll, shows current frame, not smooth
function checkScroll() {
  ...
  if(actualScrollTop != scrollTop) {
    ...
    // for static mode, need to only adjust scroll position at certain points
    // shouldn't be hit at all
    if(notAnimated && !stepper){
      findClosestScrollPosition();
      if(recordedPositionsIndex != scrollPositionsIndex){
        recordedPositionsIndex = scrollPositionsIndex;
        recordTime("scroll " + (recordedPositionsIndex + 1));
      }
      Dispatcher.trigger("scroll", scrollPositions[scrollPositionsIndex]);
    // for all of our conditions, sometimes record events
    }else{
      if(!stepper){
        findClosestScrollPosition();
      }
...
function draw() {
  ...
  // seems to animate between scrolling sessions smoothly!
  if(actualScrollTop != scrollTop) {
    newScrollTop = ZENO(scrollTop, actualScrollTop);
...
// set zeno speed as a variable
var zenoSpeed = 0.85;
var zenoThresh = 0.05;

// seems to be a smooth transition function (why zeno?)
var ZENO = function(current, actual) {
  var remainder = (current - actual) * zenoSpeed;
  if(Math.abs(remainder) < zenoThresh) {
    return actual;
  } else {
...
// scrolling hint disabled permanently
...
var ScatterPlotPointBars = Backbone.View.extend({
  ...
  render: function() {
    ...
    // hacky fix to avoid partial scroller bugs
    if(parseFloat(this.rect.attr('width')) != this.width) {
      this.rect
        .attr('rx', this.radius)
        .attr('ry', this.radius)
        .attr('width', (this.width < 0) ? 0 : this.width);
    }
  }
})
  ...
  render: function() {
    ...
    this.overlays
      ...
      .attr('width', function(d) {
        // hacky fix to avoid partial scroller bugs
        if (d.axis == 'x') {
          return (d.actualLength < 0) ? 0 : d.actualLength;
        } else {
          var value = (d.end - d.start) * v.sideLength;
          return (value < 0) ? 0 : value;
        }
      })
      .attr('height', function(d) {
        if (d.axis == 'x') {
          var value = (1 - (d.end - d.start)) * v.sideLength;
          return (value < 0) ? 0 : value;
        } else {
          return (d.actualLength < 0) ? 0 : d.actualLength;
        }
...
var SplitQualityView = Backbone.View.extend({
  initialize: function(args) {
    ...
    // buggy hack to fix the issue with the partial scroller
    if(studyState === "A" && (mainCondition === "cond4" || mainCondition === "cond3")){
      this.$furtherSection = $("#up-next");
    }
    ...
    // Accuracy Indicator
    // hack for partial scroller story issue
    var initialAccuracy = 1;
    if(studyState === "B" && (mainCondition === "cond4" || mainCondition === "cond3")){
  ...
  setSplitPoint: function(splitValue) {
    if(!this.splitData || splitValue != this.splitData.splitValue) {
      ...
      if(this.accuracyIndicator) {
        ...
        // hack for partial scroller story issue
        if(studyState === "B" && (mainCondition === "cond4" || mainCondition === "cond3")){
          this.accuracyIndicator.setAccuracyRate("82");
        }
      }
...
var AnimatedClassifierView = Backbone.View.extend({
  ...
  handleResize: function() {
    ...
    // Layout Stuff
    // special case for part vis scroller - must hack this
    var minWidth = this.bounds.width - this.paddingRight;
    if(minWidth < 0){
      minWidth = 0;
    }
    this.svg
    ...
    var treeExtent = {
      ...
      y: [this.bounds.height * zenoThresh + this.ballSpacing, this.bounds.height * 0.60]
    }
    ...
    this.funnelStart = {
      ...
      y: this.bounds.height * zenoThresh
    }
...
var SplitQualitySticky = new StickyDivView({
  ...
  bottomFn: function() { 
    // special hack for the partial scroller for a bug fix
    if(studyState === "A" && mainCondition === "cond4"){
      return $("#up-next").offset().top;// + window.innerHeight * 7.447;
    }else if(studyState === "A" && mainCondition === "cond3"){
      return $("#up-next").offset().top * 1.5;
    }
...
// whenever window resizes, regenerate our scroll positions accordingly and update stepper if needed
...
// array for fixed scroll positions & their heights (for static images)
var scrollPositions = [];
var scrollPositionsHeight = [];
// dynamically fills in these steps based on the 'steps' class
var scrollPositionNames = [];
$(".step").each(function(){
  scrollPositionNames.push("#" + this.id);
})
var scrollPositionsIndex = 0;

// populate fixed scroll positions array & heights for static images
function generateScrollPositions(){
  scrollPositions = [];
  scrollPositionsHeight = [];
  for(var i = 0; i < scrollPositionNames.length; i++){
    var scrollPos = $(scrollPositionNames[i]).offset().top;
    var scrollHei = $(scrollPositionNames[i]).height();
    // make adjustment for control panel
    if(i != 0){
      scrollPos -= 75;
    }else{
      scrollPos = 0;
    ...
    scrollPositionsHeight.push(scrollHei);
  }
}
...
// easing: slow-in, slow-out or not
var easing = true;

// interaction variable
var interactions = true;

// discrete control of machine learning transitions
$(window).keydown(function(e) {
  var key = e.keyCode;
  // use arrow keys to navigate a stepper
  // disable use for now
  if(key === 39 || key === 37){
    if(stepper){
      // updateWindowPosition((key === 39), (key === 37));
    }
  }
})

// get the closest scroll position (rounded down) to current scroll position
function findClosestScrollPosition(){
  for(var i = 1; i < scrollPositions.length - 1; i++){
    // offset position in order to get text aligned in center on event fire
    if(scrollPositions[i] > (actualScrollTop + window.innerHeight / 2)){
      scrollPositionsIndex = i - 1;
      break;
    }
    if(i == scrollPositions.length - 1 - 1){
      scrollPositionsIndex = i;
    }
  }
}

// window position updater function, based on scrollPositionIndex & array
function updateWindowPosition(inc, dec){

  // remove old box
  $(".stepper").removeClass("stepper");

  // hide all steps by default!
  $(".step").addClass("non-stepper");

  // increment or decrement our index, accordingly
  // handle different cases depending on whole story (F), first (A) or second (B) chapter
  if(inc){
    scrollPositionsIndex++;
    if(studyState === "F"){
      if(scrollPositionsIndex >= scrollPositions.length - 1){
        scrollPositionsIndex = scrollPositionsIndex % (scrollPositions.length - 1);
      }
    }else if(studyState === "A"){
      if(scrollPositionsIndex === 14){
        scrollPositionsIndex = 28;
      }else if(scrollPositionsIndex > 14){
        scrollPositionsIndex = 0;
      }
    }else if(studyState === "B"){
      if(scrollPositionsIndex >= scrollPositions.length - 1){
        scrollPositionsIndex = 14;
      }
    }
  }else if(dec){
    scrollPositionsIndex--;
    if(studyState === "F"){
      ...
      if(scrollPositionsIndex === 27){
        scrollPositionsIndex = 13;
      }else if(scrollPositionsIndex < 0){
        scrollPositionsIndex = 28;
      }
    }else if(studyState === "B"){
      if(scrollPositionsIndex < 14){
  ...
  // record time for the current step:
  recordTime("step " + (scrollPositionsIndex + 1));

  // update slider position
  var percentage = scrollPositionsIndex / (scrollPositions.length - 1 - 1) * 100;
  if(studyState === "A"){
    percentage = scrollPositionsIndex / 14 * 100;
  }else if(studyState === "B"){
    percentage = (scrollPositionsIndex - 14) / 14 * 100;
  }

  // cannot always just update slider, so catch errors
  try{
    $("#stepper-slider")[0].MaterialSlider.change(percentage);
  }catch(error){
    console.log(error);
  }

  // update our window position
  var newScrollPosition = scrollPositions[scrollPositionsIndex];

  // reset our text states every step
  $("div.step").css("margin-top", "0px");
  
  // reset all text whitespace in a step
  $("div.step  hr.whitespace").css("display", "block");

  // make sure the vis is shown (except last step below)
  $("#train-vs-test").css("opacity", 1);

  // manual adjustment of scroll position & text for stepper states!
  // standardize by page length
  var factor = scrollPositions[1] - scrollPositions[0];
  if(studyState === "B"){
    factor = scrollPositions[14] - scrollPositions[0];
  }
  var ratio = 0;
  if(scrollPositionsIndex === 4){
    ratio = 270 / 820;
  }else if(scrollPositionsIndex === 5){
    ratio = -100 / 820;
    if(studyState === "B"){
      ratio = -600 / 820;
    }
  }else if(scrollPositionsIndex === 6){
    ratio = 240 / 820;
  }else if(scrollPositionsIndex === 7){
    ratio = 270 / 820;
  }else if(scrollPositionsIndex === 9){
    ratio = 270 / 820;
  }else if(scrollPositionsIndex === 13){
    if(studyState === "B"){
      // ratio = 820 / 820;
    }
  }else if(scrollPositionsIndex === 14){
  ...
  }else if(scrollPositionsIndex === 15){
    ratio = -150 / 820;
    if(studyState === "B"){
      ratio = -820 / 820;
    }
  }else if(scrollPositionsIndex === 16){
    ratio = 50 / 820;
  }else if(scrollPositionsIndex === 17){
    ratio = -100 / 820;
  }else if(scrollPositionsIndex === 19){
    ratio = 150 / 820;
  }else if(scrollPositionsIndex === 25){
    ratio = -170 / 820;
  }else if(scrollPositionsIndex === 24){
    ratio = 290 / 820;
    if(studyState === "B"){
      ratio = 1400 / 820;
    }
  ...
  }else if(scrollPositionsIndex === 26){
    ratio = -250 / 820;
    if(studyState === "B"){
      ratio = -1640 / 820;
    }
  }else if(scrollPositionsIndex === 27){
    // hide the visualization on final step
    $("#train-vs-test").css("opacity", 0);
  }else if(scrollPositionsIndex === 28){
    ratio = -500 / 820;
    // hide the visualization on final step
    $("#train-vs-test").css("opacity", 0);
  }
  // hide decision tree & pie charts manually
  if(studyState === "A"){
    $("#decision-tree").css("opacity", 0);
    if(scrollPositionsIndex === 28){
      $("#split-quality").css("opacity", 0);
    }else{
      $("#split-quality").css("opacity", 1);
    }
  }

  // manual hack for one step of stepper
  if(studyState === "A" && scrollPositionsIndex === 5){
    newScrollPosition = scrollPositions[scrollPositionsIndex - 1];
    ratio = 270 / 820;
  }

  newScrollPosition += ratio * factor;
  $(scrollPositionNames[scrollPositionsIndex]).css("margin-top", (ratio * factor) + "px");
  if(studyState === "A" && scrollPositionsIndex === 5){
    var movedText = -(scrollPositions[scrollPositionsIndex] - scrollPositions[scrollPositionsIndex - 1]) + (ratio * factor);
    $(scrollPositionNames[scrollPositionsIndex]).css("margin-top", movedText + "px");
  }

  // remove all text whitespace in a step
  $(scrollPositionNames[scrollPositionsIndex] + " hr.whitespace").css("display", "none");

  $window.scrollTop(newScrollPosition);

  // un-hide our current step!
  $(scrollPositionNames[scrollPositionsIndex]).removeClass("non-stepper");

  // add new css box
  $(scrollPositionNames[scrollPositionsIndex]).addClass("stepper");

  // update text indicator
  if(studyState === "F"){
    $("#slider-steps").text(scrollPositionsIndex + 1);
  }else if(studyState === "A"){
    var textStep = scrollPositionsIndex + 1;
    if(textStep === 29){
      textStep = 15;
    }
    $("#slider-steps").text(textStep);
  }else if(studyState === "B"){
    var textStep = scrollPositionsIndex - 14 + 1;
    $("#slider-steps").text(textStep);
  }
...
/*  ga('send', 'screenview', {
    ...
    'screenName': scrollPositionsIndex
  });*/
}

// stepper functionality (buttons)
$("#stepper-prev").on("mouseup", function(e){
  updateWindowPosition(false, true);
})
$("#stepper-next").on("mouseup", function(e){
  updateWindowPosition(true, false);
})

// stepper functionality (slider)
$("#stepper").on("change", function(e){
  var sliderValue = $("#stepper input")[0].value;
  scrollPositionsIndex = Math.floor(sliderValue / 100 * (scrollPositions.length - 1 - 1))
  updateWindowPosition();
})
...
// attach event listener for: animated vs static transitions
$("#switch-2").on("change", function(e){
  notAnimated = !$("#switch-2")[0].checked;
  if(notAnimated){
    recordTime("deactivate-animation");
    showNoAnimation();
  }else{
    recordTime("activate-animation");
    showAnimation();
  }
})

// stepper/scroller functions
function showStepper(){
  // disable scrolling
  $('body').addClass('stop-scrolling');
  // enable buttons
  $("#stepper").css("display", "block");
  // disable keep-scrolling --> disabled permanently
  // $("#keep-scrolling").css("display", "none");
  // disable text to scroll
  $("#set-up p em").css("display", "none");
  // update speed of animation
  if(zenoSpeed != 0){
    zenoSpeed = 0.95;
  }
  if(!easing){
  ...
  findClosestScrollPosition();
  updateWindowPosition();
  var sliderTotal = scrollPositions.length - 1;
  if(studyState === "A"){
    sliderTotal = 15;
  }else if(studyState === "B"){
    sliderTotal = 15;
  }
  $("#slider-total").text(sliderTotal);
  // change text
  $("#switch-1").parent().find(".mdl-switch__label").text("stepper");
  stepper = true;
}
function hideStepper(){
  // unhide all text!
  $(".non-stepper").removeClass("non-stepper");
  // disable stepper box
  $(".stepper").removeClass("stepper");
  // enable scrolling
  $('body').removeClass('stop-scrolling');
  ...
  // enable keep-scrolling --> disabled permanently
  // $("#keep-scrolling").css("display", "block");
  // enable text to scroll
  $("#set-up p em").css("display", "inline");
  // update speed of animation
  if(zenoSpeed != 0){
    zenoSpeed = 0.85;
  }
  ...
  // change text
  $("#switch-1").parent().find(".mdl-switch__label").text("scroller");
  stepper = false;
}
...
function toggleSettings(){
  if(debugMode){
    var menu = document.querySelector("#controls").classList;
...
// attach event listener for: scroller vs stepper
$("#switch-1").on("change", function(e){
  stepper = !$("#switch-1")[0].checked;
  if(stepper){
    recordTime("activate-stepper");
    showStepper();
  }else{
    recordTime("activate-scroller");
    hideStepper();
  }
})

// attach event listener for: chapters
function filterChapter(chapter){
  if(chapterFilter){
    var allChapters = ["chap1", "chap2", "chap3"];
    $(".invisible").removeClass("invisible");
    for(var c in allChapters){
      $("." + allChapters[c]).addClass("invisible");
    }
    $("." + chapter).removeClass("invisible");
  }
}
$(".chapters").on("change", function(e){
  filterChapter(e.target.id);
})

// clear visualizations for static versions
function clearAllVis(){
  $(".static-container").css("opacity", 0).css("visibility", "hidden").css("display", "none");
	$("#shadow-scatterplot").remove();
}
// reshow visualizations
function showAllVis(){
  $(".static-container").css("opacity", 1).css("visibility", "visible").css("display", "block");
}

// place static image locations & var
var staticCondition = false;
function placeStaticImages(){
  var staticToTextMapping = [0, 1, 2, 3, 4, -1, 6, 7, 8, 10, 12, 14, -1, -1, -1, 16, -1, -1, 21, -1, -1, 24, -1];
  generateScrollPositions();
  for(var i in staticToTextMapping){
    $(".static-image-step#static-" + i).css("top", scrollPositions[staticToTextMapping[i]]);
    var imageHeight = scrollPositionsHeight[staticToTextMapping[i]];
    // manually grow images to line up with multiple sections!
    if(i == 8){
      imageHeight += scrollPositionsHeight[9];
    }else if(i == 9){
      imageHeight += scrollPositionsHeight[11];
    }else if(i == 10){
      imageHeight += scrollPositionsHeight[13];
    }else if(i == 11){
      imageHeight += scrollPositionsHeight[15];
    }else if(i == 15){
      imageHeight += scrollPositionsHeight[17] + scrollPositionsHeight[18] + scrollPositionsHeight[19] + scrollPositionsHeight[20];
    }else if(i == 18){
      imageHeight += scrollPositionsHeight[22] + scrollPositionsHeight[23];
    }else if(i == 21){
      imageHeight += scrollPositionsHeight[25] + scrollPositionsHeight[26];
    }
    // sometimes don't limit the image height
    // manually adjust width of a few sections as well
    if(i != 4 & i != 10){
      $(".static-image-step#static-" + i + " .static-image").css("max-height", imageHeight);
    }else{
      if(i == 4){
        var imageWidth = $(".static-image-step#static-3 .static-image").css("width");
        $(".static-image-step#static-4 .static-image").css("width",imageWidth);
        $(".static-image-step#static-4 .static-image").css("margin-top", "-5vh");
      }else if(i == 10){
        var imageWidth = $(".static-image-step#static-9 .static-image").css("width");
        $(".static-image-step#static-10 .static-image").css("width",imageWidth);
        $(".static-image-step#static-10 .static-image").css("margin-top", "-5vh");
      }
    }
  }
}

// move text over and back
function moveBodyText(){
  $(".container#main .col-xs-4").removeClass("col-xs-4").addClass("col-xs-8").addClass("col-xs-offset-2").addClass("temp4");
  $(".container#main .col-xs-5").removeClass("col-xs-5").addClass("col-xs-8").addClass("col-xs-offset-2").addClass("temp5");
}
function moveBodyTextDefault(){
  $(".container#main .temp4").removeClass("col-xs-8").removeClass("col-xs-offset-2").addClass("col-xs-4");
  $(".container#main .temp5").removeClass("col-xs-8").removeClass("col-xs-offset-2").addClass("col-xs-5");
  $("div.step").css("margin-top", "0px");
  $("div.step  hr.whitespace").css("display", "block");
}

// hide whitespace between sections
function hideWhitespace(){
  $(".whitespace").css("display", "none");
  $(".whitespace-to-keep").css("display", "block");
  $("h2").css("margin-top", "18vh");
}
...
// show or hide text-only altered text
function showAlteredText(){
  $(".altered-text").css("display", "inline");
  $(".text-to-alter").css("display", "none");
}
function hideAlteredText(){
  $(".altered-text").css("display", "none");
  $(".text-to-alter").css("display", "inline");
}

// attach event listener for: 4 study conditions (text, static, stepper, scroller)
// main condition is defined in index.html
function switchConditions(cond){
  mainCondition = cond;
  // condition for just text or for text with static vis
  if(cond === "cond1"){
    ...
    generateScrollPositions();
    findClosestScrollPosition();
    $(".static-image-step").addClass("hide");

  // test hiding the vis and unhiding static images
  }else if(cond === "cond2"){
    staticCondition = true;
    clearAllVis();
    hideStepper();
    ...
    generateScrollPositions();
    findClosestScrollPosition();
    $(".static-image-step").removeClass("hide");
    placeStaticImages();
...
$(".conditions").on("change", function(e){
  switchConditions(e.target.id);
})

// initialize main condition
$(document).ready(function(){
  // remove scrolling indicator permanently
  $("#keep-scrolling").remove();

  switchConditions(mainCondition);
});

// attach event listener for: easing (slow-in, slow-out) or not
// this is less than ideal, changing zenoSpeed in multiple places now!
// TODO stepper is less than ideal but oh well... the easing is really linked too inherently with the length of the animation which is a problem!
$("#switch-3").on("change", function(e){
  easing = $("#switch-3")[0].checked;
  if(!easing){
    zenoSpeed = 0;
    if(stepper && !notAnimated){
      zenoSpeed = 0.1;
    }
    zenoThresh = 0.01;
  }else{
...
// attach event listener for: interactions or not
$("#switch-4").on("change", function(e){
  interactions = $("#switch-4")[0].checked;
})

// reduce whitespace
// $(".whitespace").each(function(){this.style.height = "5vh"})

// detect when user reaches bottom, time it and send to google analytics
window.onscroll = function(e) {
  if((window.innerHeight + window.scrollY) >= $(document).height()){
    recordTime("end-of-story");
    var startTime = pageTimingData[2];
    var endTime = pageTimingData[pageTimingData.length - 1];
    var timeToRead = endTime[Object.keys(endTime)[0]] - startTime[Object.keys(startTime)[0]];
    ga('send', 'timing', {
      'timingCategory': 'scroller',
      'timingVar': 'story-time',
      'timingValue': timeToRead
    });
    ga('send', {
      hitType: 'event',
      eventCategory: 'scroller',
      eventAction: 'finish',
      eventLabel: 'time-taken',
      eventValue: timeToRead
    });
    // 3rd value (eventValue) is when we agreed to consent form, see how long it took to read!
  }
};
...
// function to store timing events into our data structure
function recordTime(event){
  // store time in seconds (microsecond precision)
  var t = Math.round(performance.now()) / 1000.0;
  var e = {};
  e[event] = t;
  if(debugMode){
    console.log(event + ": " + t);
  }
  pageTimingData.push(e);
}

// begin with chapter 1 (if filtering at all)
// filterChapter("chap1");

// perform captcha check
function captchaCheck(){
  var first = document.querySelector(".captcha-checker").contentWindow.document.querySelector("#q-c-1-4").checked;
  var second = false;
  var third = false;
  if(studyState === "F"){
    if(mainCondition === "cond1"){
      second = document.querySelector(".captcha-checker").contentWindow.document.querySelector("#q-c-3-2").checked;
    }else{
      second = document.querySelector(".captcha-checker").contentWindow.document.querySelector("#q-c-2-4").checked;
    }
    third = true;
  }else{
    second = document.querySelector(".captcha-checker").contentWindow.document.querySelector("#q-c-3-2").checked;
    third = document.querySelector(".captcha-checker").contentWindow.document.querySelector("#q-c-2-4").checked;
  }
  return first && second && third;
}

// create user restriction cookie
function restrictUser(){
  if(!debugMode){
    document.cookie = "access = restricted";
  }
  rejectUser();
}
...
function rejectUser(){
  $("body").remove();
  $("html").append("<p class='pointer'>We are sorry, but you are ineligible for this task. Please return the HIT.</p>");
}

// only keep from getting to page in future
function restrictUserInFuture(){
  if(!debugMode){
     document.cookie = "access = restricted";
  }
   $("div#main").remove();
   $(".static-image-step").remove();
...
function canUserAccessPage(){
  if(document.cookie){
    var cookie = document.cookie.split(";").map(function(e){
      return e.trim().split("=");
    });
    for(var c in cookie){
      if(debugMode){
      ...
      var key = cookie[c][0];
      var value = cookie[c][1];
      if(key === "access" && value === "restricted"){
        return false;
      }
    }
  }
...
// force a modal for user to adjust page resolution
function isWindowBigEnough(){
  var minWidth = 1203;
  var minHeight = 880;
