...
function bar(){
  ...
  let yComparison = d3.scaleLinear()
    .range([height, height - ((50 + 8) * 3)]);
  yComparison.domain([0, 3]);
  let yComparisonAxis = d3.axisLeft()
    .scale(yComparison)
    .tickValues([1, 2, 3])
    .tickFormat(d3.format(","));
  let yComparison2 = d3.scaleLinear()
    .range([height, height - ((50 + 8) * 2)]);
  yComparison2.domain([0, 2]);
  let yComparisonAxis2 = d3.axisRight()
    .scale(yComparison2)
    .tickValues([1, 2])
    .tickFormat(d3.format(","));
  ...
  function bar(selection){
    ...
    selection.each(function(d, i){
      ...
      chart.selectAll(".bar")
        ...
        .enter().append("rect")
          ...
          }).attr("x", x(d.gender)).attr("width", x.bandwidth())
    ...
    chart.append("g")
      .attr("class", "legend")
      .selectAll(".age").data(data.slice().reverse()).enter().append("rect")
        .attr("class", "age")
        .attr("x", x.bandwidth() * 2 + 100)
        .attr("y", function(d, i){
    ...
    chart.select(".legend").selectAll(".ageText").data(data.slice().reverse()).enter().append("text")
      .attr("class", "ageText")
      .attr("x", x.bandwidth() * 2 + 130)
      .attr("y", function(d, i){
  ...
  bar.scaleAxisSmall = function(elements){
    // elements.call(yAxis);
    y.domain([0, 7.75]);
    // TODO bug going back, this won't fix...'
  ...
  bar.allByGender = function(elements){
    elements
      .attr("y", function(d, i, j){
        let adjustment = (d.age === "child") ? data[i + 1].quantity - 0.1 : 0;
  ...
  bar.moveApart = function(elements){
    ...
    elements
      .attr("y", function(d, i, j){
        let genderIndex = (d.gender == "F") ? 1 : 3;
        let yStacked = (d.age == "adult") ? d.quantity : (data[genderIndex].quantity + d.quantity + 0.75);
        return y(yStacked);
      })
      .attr("height", function(d, i, j){
        if(!d.family){
  ...
  bar.moveToAge = function(elements){
    ...
    elements.attr("x", function(d){
      ...
      if(!d.family){
        let adjustment = (d.gender === "F") ? data[i + 2].quantity - 0.02 : 0;
        return y(d.quantity + adjustment);
      }
    }).attr("height", function(d, i, j){
      if(!d.family){
  ...
  bar.legendPosition = function(elements){
    elements.attr("x", x.bandwidth() * 2 + 130)
      .attr("y", function(d, i){
        return height + 60 - 40 * i + 18;
      });
  };
